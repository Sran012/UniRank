  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 sm:gap-8 w-full max-sm:scale-[0.95] max-sm:origin-center sm:py-8">
      <div className="text-center relative shrink-0">
        <div className="inline-block bg-[var(--danger)] text-white px-3 py-1 text-sm sm:text-lg font-black uppercase border-[3px] border-[var(--foreground)] shadow-[4px_4px_0_var(--foreground)] rotate-2 mb-2 sm:mb-4">
          WARNING: EXPOSED
        </div>
        <h1 className="text-4xl sm:text-6xl font-black uppercase tracking-tighter break-all bg-[var(--foreground)] text-[var(--primary)] px-5 py-3 border-[4px] border-[var(--foreground)] shadow-[6px_6px_0_var(--warning)] text-center leading-[0.9]">
          {data.name}
        </h1>
      </div>

      <div className="neobrutal-box w-full p-4 sm:p-8 flex flex-col gap-4 sm:gap-6 bg-[var(--primary)] relative mt-2 shrink-0 border-[4px] shadow-[6px_6px_0_var(--foreground)] sm:shadow-[10px_10px_0_var(--foreground)]">
        <div className="space-y-4 sm:space-y-8 relative z-10 w-full font-sans">
          <div className="flex items-center justify-between border-[4px] border-[var(--foreground)] bg-white px-3 py-2 sm:px-5 sm:py-3 shadow-[4px_4px_0_var(--foreground)]">
            <span className="text-sm sm:text-xl font-extrabold uppercase text-[var(--foreground)]">Rank</span>
            <span className="text-3xl sm:text-5xl font-black text-[var(--danger)] leading-none">#{data.rank}</span>
          </div>
      ...
      <div className="w-full flex flex-col items-center gap-3 shrink-0">
        <button
          onClick={onContinue}
          className="neobrutal-button-secondary w-full py-4 sm:py-6 text-xl sm:text-3xl uppercase tracking-widest mt-2 border-[4px] shadow-[6px_6px_0_var(--foreground)] sm:shadow-[8px_8px_0_var(--foreground)]"
        >
          Check My Own Rank 🎯
        </button>
      </div>
